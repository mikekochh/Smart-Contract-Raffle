...
contract RaffleTest is Test {
    ...
    function testEmitsEventOnEntrance() public {
        vm.prank(PLAYER);
        // checkTopic1, checkTopic2, checkTopic3, checkData, address emitter
        vm.expectEmit(true, false, false, false, address(raffle));
        emit EnteredRaffle(PLAYER); // we need to define and emit the event that we expected to be emitted in the function call below
        raffle.enterRaffle{value: entranceFee}();
    }
    ...
    // write these tests yourself, then check with video if they are correct
        ...
        raffleEnteredAndTimePassed
    {
        ...
        bytes32 requestId = entries[1].topics[1]; // getting the second event emitted, and the second topic refers to the first parameter. all logs are recorded in bytes32
        //bytes32 requestId = entries[0].topics[2]; // also does the same thing, but is looking at the first event emitted
        ...
        skipFork
    {
        ...
        // vm.expectEmit(true, false, false, false, address(raffle)); // could not figure this out. Test to make sure event was emitted
